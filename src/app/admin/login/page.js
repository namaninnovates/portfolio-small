  ...
  return (
    <div className="min-h-screen bg-surface-container-low flex items-center justify-center p-4">
      <div className="bg-background border-4 border-on-background neo-shadow max-w-md w-full p-8 relative">
        ...
        <h1 className="font-display-md text-headline-md uppercase mb-8 mt-4">Admin Login</h1>
        
        <form action={formAction} className="space-y-6" noValidate>
          <div>
            <label className="block font-label-md uppercase mb-2">Password</label>
            <input 
              type="password" 
              name="password"
            ...
            />
            {state?.error && (
              <p className="text-error font-label-md mt-2 flex items-center gap-1">
          ...
          <button 
            type="submit" 
            disabled={isPending}
            className="w-full bg-on-background text-background font-label-lg uppercase border-4 border-transparent p-3 hover:bg-cobalt hover:text-on-primary hover:border-on-background transition-all active:translate-y-1 disabled:opacity-50"
          >
